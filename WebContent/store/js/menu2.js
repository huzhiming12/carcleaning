window.onload = function() {
	...
	$(".addBtn2")
			.on(
					...
					function() {
						var a;
						add: for (var i = 0; i < $(this).parents(".tc").find(
								".addInp").length; i++) {
							a = $(this).parents(".tc").find(".addInp").eq(i)
									.attr("statu");
							if (a != "ok") {
								alert("请输入正确信息！")
								break add;
							} else if (i == $(this).parents(".tc").find(
									".addInp").length - 1) {
								if (a != "ok") {
									alert("请输入正确信息！")
									break add;
									...
									// "");
									// $(this).parents(".tc").find(".addInp")
									// .attr("statu", "no");
									// $(this).parents(".tc").find(".addInp").css(
									// "border-color", "#cdcdcd");
									var ifm = $('<iframe width="0" height="0" id="iframe" frameborder="0" name="iframe">');
									ifm.appendTo($('body'));
									$(".addFrom").submit();
									ifm.load(function() {
										var jsonStr = $(this).contents().find(
												"*").first().text();
